...
public class LeanBehaviour : MonoBehaviour
{
    ...
    [SerializeField] private SkateboardBehaviour skateboardBehaviour;

    // Update is called once per frame
    void Update()
    {
        if (!skateboardBehaviour) return;
            
        Vector3 localRotation = transform.localRotation.eulerAngles;
        localRotation.z = Mathf.Clamp(skateboardBehaviour.turnAmount * maxAngle, -maxAngle, maxAngle);
